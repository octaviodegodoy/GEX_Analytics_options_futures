...
def main():
    ...
    # Resolve current + previous WIN symbol
    try:
    ...
    if not win_symbol:
        print("[!] No WIN contract resolved — aborting")
        return
    ...
    try:
        ...
        print(f"[i] Kalman mapper built (intraday 15m) using {win_symbol}")
    except Exception as e:
        print(f"[!] Mapper with {win_symbol} failed: {e}")
    if mapper is None:
